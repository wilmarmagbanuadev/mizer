...
if ( ! function_exists( 'blank_posted_by' ) ) :
	...
	function blank_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'mizer' ),
...
if ( ! function_exists( 'blank_entry_footer' ) ) :
	...
	function blank_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'mizer' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'mizer' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'mizer' ),
						array(
							'span' => array(
								'class' => array(),
							),
		...
		edit_post_link(
			sprintf(
				wp_kses(
					...
					__( 'Edit <span class="screen-reader-text">%s</span>', 'mizer' ),
					array(
						'span' => array(
							'class' => array(),
						),
